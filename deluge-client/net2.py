...
def multiControllerNet():
    ...
    right_switch = [net.addSwitch('R%d'%(n+1)) for n in range(tmp-1)]
    ...
    hosts2 = [ net.addHost( 'h%d' % (n+1) ,ip='10.0.0.%s'%(n+1+20)) for n in range(tmp) ]
    ...
    #for k in sdn_switch:
    #    k.start([c3])
    right_switch[0].cmdPrint('ovs-vsctl add-port '+right_switch[0].name+' '+right_switch[0].name+'-gre1 -- set interface '+right_switch[0].name+'-gre1 type=gre options:remote_ip='+NODE1_IP)
    for k in hosts2:
        k.cmdPrint('ip link set mtu 1454 dev '+k.name+'-eth0')

    natIP='10.0.0.254'
    for k in hosts2:
        k.setDefaultRoute('via %s'%natIP)
